...
export const ViewPage = () => {
    ...
    const getDataAd = useCallback(async () => {
        try {
            const fetched = await request(`/api/ad/get_data/${schoolId}`, 'GET', null);
            setAd({ad: fetched.ad});
            /*if (adImage.url.indexOf('/api/ad/get/') === -1) {
                setAdImage({...adImage, url: `/api/ad/get/${fetched.ad[countAd.count]._id}`, date: new Date()});
